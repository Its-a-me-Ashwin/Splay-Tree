...
/*  MOVE ALL OF THESE FUNCTION IN TO THE CLASS TO FIX BUGS
...
{  
    if (root == NULL || root->key == key)  // head
        return root;  
    if (root->key > key)  //left 
    {   
        if (root->left == NULL) // no element
        {
            return root;  
        }  
    ...
    {   
        if (root->right == NULL) //no element
        {
            //printf("element gone");
            return root;  
        }
        if (root->right->key > key) //rihgt left 
        {  
            root->right->left = splay(root->right->left, key);  
            if (root->right->left != NULL)  
                root->right = rightRotate(root->right);  
        }  
        else if (root->right->key < key)// right 
        {  
...
node * insert_bst (node* head, int data)
{
	...
	if (data < head->key) head->left = insert_bst(head->left,data);
	else if (data > head->key) head->right = insert_bst(head->right,data);
	//else printf("Error");
	return head;
}
...
node * delete_bst (node* root,int data)
{
	...
	if (root->key > data) root->left = delete_bst(root->left,data);
	else if (root->key < data) root->right = delete_bst(root->right,data);
	else // found
	{
		if (root->left == NULL)
		{
...
node * get_parent (node * head,int data)
{	
	node *temp; // put in main ()
	if (head == NULL || head->key == data) return NULL;
	while(head != NULL)
	{
		temp = head;			
		if (data < head->key) head = head->left;
		else if (data > head->key) head = head->right;
		else return temp;
	}
	return NULL;
}
int check(node *head,int data)
{
	if (head == NULL) return 0;	// spalyed
	if (head->key == data) return 1;
	else if (head->key > data) return check(head->left,data);
	else return check(head->right,data);
}



node * in_order_scucessor (node * head)
{
